...
## 🚀 How to download and run blitz-swarm on Windows
...
4. In the Releases section, find the latest version for Windows. It should be an `.exe` file or a `.zip` file with an `.exe` inside.

5. Download the `.exe` file to your computer. Save it to your Downloads folder or Desktop.

6. Once downloaded, find the file and double-click it.

7. If Windows shows a security warning, choose to run the file anyway.

8. The program may open a window or run in the background. Follow any on-screen instructions.

9. If the program ends or closes unexpectedly, try running it again. You may see some logs or messages showing what is happening.

You do not need to install anything else to get started.

## 📂 Running blitz-swarm step by step
...
- To stop blitz-swarm, close the command window or press `Ctrl + C` in it.
- The program uses your computer resources, so performance depends on your hardware.

## 🔧 Optional setup: Running from a ZIP file

Sometimes, the download may come as a `.zip` file.

1. Right-click the `.zip` file and select “Extract All…”.

2. Choose a folder where you want to keep the files.

3. Open the folder and double-click the `.exe` file inside.

4. This starts the program in the same way as above.

This method is useful if you want to keep the program in a specific folder.

## 🛠️ What does blitz-swarm do?
...
- If the window closes immediately, run `cmd.exe`, navigate to the folder, and run the program from the command line to see error messages.
